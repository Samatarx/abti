...
Any reminder must be optional. It should only suggest that the user may want an Abti learning debrief. It must not generate the learning debrief without consent, and it must not store, log, upload, analyse, or inspect prompts, code, summaries, learning tags, secrets, or sensitive data.

Only mention this after Abti has produced a learning debrief, the user asks why Abti did not trigger, or the user asks how to make Abti trigger more reliably. If the user says no, respect that and do not ask again in the same session.
