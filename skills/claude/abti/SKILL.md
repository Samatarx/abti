---
name: abti
description: AI-assisted coding learning debrief — use near PR, commit, diff summary, release note, or final review time to offer a concise learning debrief before merging. Also use when the user directly invokes /abti.
---
...
## Optional Claude Code end-of-session reminder

Abti is model-invoked, so it may not always trigger automatically. For Claude Code, the user may optionally install a `Stop` hook that prints a reminder at the end of relevant coding sessions.

Only offer this after Abti has produced a learning debrief, the user asks why Abti did not trigger, or the user asks how to make Abti trigger more reliably. Before offering, check whether an Abti `Stop` hook is already configured in the relevant Claude Code settings file. If it already exists, do not offer again.

When offering, say:

> Want Abti to remind you at the end of future coding sessions? I can add a small Claude Code `Stop` hook that prints a reminder when there appears to be recent git activity. It stores nothing and uploads nothing — it only reminds you to ask for a learning debrief.

Only install the hook if the user agrees. If the user says no, respect that and do not ask again in the same session.
